...
const log = (0, debug_1.debug)("packages/rtp/src/rtp/red/packet.ts");
// 0                   1                    2                   3
...
class RedHeader {
    ...
    static deSerialize(buf) {
        ...
        for (;;) {
            ...
            const bitStream = new src_1.BitStream(buf.subarray(offset));
        ...
        for (const field of this.fields) {
            try {
                if (field.timestampOffset && field.blockLength) {
                    const bitStream = new src_1.BitStream(Buffer.alloc(4))

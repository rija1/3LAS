...
const log = (0, debug_1.default)("werift/rtp/rtcp/rtpfb/twcc");
...
class TransportWideCC {
    ...
    static deSerialize(data, header) {
        const [senderSsrc, mediaSsrc, baseSequenceNumber, packetStatusCount, referenceTime, fbPktCount,] = (0, src_1.bufferReader)(data, [4, 4, 2, 2, 3, 1]);
        const packetChunks = [];
        const recvDeltas = [];
        let packetStatusPos = 16;
        for (let processedPacketNum = 0; processedPacketNum < packetStatusCount;) {
            const type = (0, src_1.getBit)(data.slice(packetStatusPos, packetStatusPos + 1)[0], 0, 1);
            let iPacketStatus;
            switch (type) {
                case PacketChunk.TypeTCCRunLengthChunk:
                    {
                        ...
                        if (packetStatus.packetStatus ===
                            ...
                            (0, range_1.default)(packetNumberToProcess).forEach(() => {
    ...
    serialize() {
        const constBuf = (0, src_1.bufferWriter)([4, 4, 2, 2, 3, 1], [
            this.senderSsrc,
...
class RunLengthChunk {
    ...
    static deSerialize(data) {
        const packetStatus = (0, src_1.getBit)(data[0], 1, 2);
        const runLength = ((0, src_1.getBit)(data[0], 3, 5) << 8) + data[1];
        return new RunLengthChunk({ type: 0, packetStatus, runLength });
    }
    serialize() {
        const buf = new src_1.BitWriter2(16)
            .set(0)
...
class StatusVectorChunk {
    ...
    static deSerialize(data) {
        ...
        let symbolSize = (0, src_1.getBit)(data[0], 1, 1);
        const symbolList = [];
        switch (symbolSize) {
            case 0:
                (0, range_1.default)(6).forEach((_, i) => symbolList.push((0, src_1.getBit)(data[0], 2 + i, 1)));
                (0, range_1.default)(8).forEach((_, i) => symbolList.push((0, src_1.getBit)(data[1], i, 1)));
                break;
            case 1:
                (0, range_1.default)(3).forEach((i) => symbolList.push((0, src_1.getBit)(data[0], 2 + i * 2, 2)));
                (0, range_1.default)(4).forEach((i) => symbolList.push((0, src_1.getBit)(data[1], i * 2, 2)));
                break;
            default:
                symbolSize = ((0, src_1.getBit)(data[0], 2, 6) << 8) + data[1];
        }
        return new StatusVectorChunk({ type, symbolSize, symbolList });
    }
    serialize() {
        const buf = Buffer.alloc(2);
        const writer = new src_1.BitWriter2(16).set(1).set(this.symbolSize);
